...
module.exports = async ({ url, tags, tagNum, depth = 1, form, charset = 'utf-8', proxy, fn }) => {
  ...
  if (proxy) {
    superagent.get(url).set(HEADER).timeout(10000).proxy(proxy).charset(charset).buffer(true).end((err, res) => {
      bound(err, res)
    })
  } else {
    superagent.get(url).set(HEADER).timeout(10000).charset(charset).buffer(true).end((err, res) => {
      bound(err, res)
    })
  }
}
